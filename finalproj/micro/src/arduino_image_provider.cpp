...
TfLiteStatus GetImage(tflite::ErrorReporter* error_reporter, int image_width,
  ...
  byte data[176 * 144 * 2]; // Receiving QCIF grayscale from camera = 176 * 144 * 1
  ...
  if (!g_is_camera_initialized) {
    if (!Camera.begin(QCIF, RGB444, 5, OV7675)) {
  ...
  TF_LITE_REPORT_ERROR(error_reporter, "Reading Camera");
  // Read camera data
